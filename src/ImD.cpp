...
void ImD::writeBMP(const std::string& fileName, const ImD& imageData)
{
    ...
    unsigned char padding_bytes[3] = {0, 0, 0}; // Изменено имя переменной, чтобы избежать конфликта с полем класса padding
...
void ImD::reader(std::string fileName, bool need_clock, bool need_contrclock, bool need_Gaus)
{
    ...
    pixels = new RGBQUAD[static_cast<size_t>(infoHeader.biHeight) * infoHeader.biWidth]; // Добавлено static_cast для корректного выделения памяти
